...
<div class="w-full px-40 flex justify-between py-8 gap-4">
    ...
    <div class=" container mx-auto px-8 py-8 w-3/4 border border-primary">
        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex flex-between gap-4 mb-4">
                <div class="mb-2 w-1/3">
                    <label for="kota" class="block text-gray-500 mb-1">Kota</label>
                    <input type="text" id="kota" name="kota" value="{{ old('kota', Auth::user()->kota) }}" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>

                <div class="mb-2 w-1/3">
                    <label for="kecamatan" class="block text-gray-500 mb-1">Kecamatan</label>
                    <input type="text" id="kecamatan" name="kecamatan" value="{{ old('kecamatan', Auth::user()->kecamatan) }}" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>

                <div class="mb-2 w-1/3">
                    <label for="desa" class="block text-gray-500 mb-1">Desa</label>
                    <input type="text" id="desa" name="desa" value="{{ old('desa', Auth::user()->desa) }}" class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
            </div>
